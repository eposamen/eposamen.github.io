var TimeKnots = {
  draw: function(id, events, options){
    ...
    var svg = d3.select(id).append('svg').attr("width", "100%").attr("height", cfg.height);
...
//                         ret = Math.floor(cfg.width/2)
//                       }
//                       linePrevious.x1 = ret + newMargin
//                       console.log("****** = "+(ret + newMargin))
//                       return ret + newMargin
//                       })
//     .attr("x2", function(d){
//                       if (linePrevious.x1 != null){
//                           return linePrevious.x1 + newMargin
//                       }
//                       if(cfg.horizontalLayout){
//                         var datum = (cfg.dateDimension)?new Date(d.date).getTime():d.value;
//                         ret = Math.floor(step*(datum - minValue ))
//                       }
//                       console.log("***** = "+(Math.floor(cfg.width/2) + newMargin));
//                       return Math.floor(cfg.width/2) + newMargin
//                       })
//     .attr("y1", function(d){
//                       var ret;
//                       if(cfg.horizontalLayout){
...
//     .style("stroke-width", cfg.lineWidth);
    ...
    .attr("xlink:href", "images/job-triangle.png")
    // .style("stroke", function(d){
    //                 if(d.color != undefined){
    //                   return d.color
    //                 }
    ...
    .attr("y", function(d){
        ...
        var datum = (cfg.dateDimension)?new Date(d.date).getTime():d.value;
        return Math.floor(step*(datum - minValue) + margin ) - (cfg.radius/2)
    })
    .attr("x", function(d){
        if(cfg.horizontalLayout){
          var datum = (cfg.dateDimension)?new Date(d.date).getTime():d.value;
      ...
      .transition().duration(150).attr("height",  function(d){if(d.radius != undefined){return Math.floor(d.radius*1.5)} return Math.floor(cfg.radius*1.5)})
                  .attr("width",  function(d){if(d.radius != undefined){return Math.floor(d.radius*1.5)} return Math.floor(cfg.radius*1.5)})
                  .attr("x", function(d){
                      if(cfg.horizontalLayout){
                        var datum = (cfg.dateDimension)?new Date(d.date).getTime():d.value;
                        var x=  Math.floor(step*(datum - minValue) + margin);
      ...
      tip.html("");
      if(d.img != undefined){
        tip.append("img").style("float", "left").style("margin-right", "4px").attr("src", "images/javascript-icon.png").attr("width", "64px");
      }
    ...
    .on("mouseout", function(){
        ...
        .transition().duration(150).attr("height", function(d){if(d.radius != undefined){return d.radius} return cfg.radius})
                      .attr("width", function(d){if(d.radius != undefined){return d.radius} return cfg.radius})
                      .attr("x", function(d){
                      if(cfg.horizontalLayout){
                        var datum = (cfg.dateDimension)?new Date(d.date).getTime():d.value;
...
svg.selectAll("text")
    .data(events).enter()
        .append("text")
          .attr("y", function(d){
              if(cfg.horizontalLayout){
                return Math.floor(cfg.height/2) + 20
              }
              var datum = (cfg.dateDimension)?new Date(d.date).getTime():d.value;
              return Math.floor(step*(datum - minValue) + margin ) - (cfg.radius/2) + 20
          })
          .attr("x", function(d){
              console.log(d);
              if(cfg.horizontalLayout){
